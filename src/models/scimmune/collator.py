...
class ScImmuneDataCollator(DefaultDataCollator):
    ...
    def _scale_expressions_inplace(self, t: torch.Tensor) -> torch.Tensor:
        # Skip if binning is on or no scaling requested
        if self.do_binning or self.scale_factor is None or self.scale_factor == 1.0:
            return t
        # Ensure float
        if not torch.is_floating_point(t):
            t = t.float()
        # Only scale real values (not sentinels)
        mask = (t != float(self.pad_value)) & (t != float(self.mask_value))
        t[mask] = t[mask] / float(self.scale_factor)
        return t
    ...
    def _call_pcpt(
        ...
        padded_expressions = self._scale_expressions_inplace(padded_expressions)
    ...
    def _call_gen(
        ...
        padded_pcpt_expressions = self._scale_expressions_inplace(padded_pcpt_expressions)
    ...
    def _call_both(
        ...
        padded_pcpt_expressions = self._scale_expressions_inplace(padded_pcpt_expressions)
        padded_gen_expressions  = self._scale_expressions_inplace(padded_gen_expressions)
        ...
        data_dict = {
            ...
            "pcpt_key_padding_mask": padded_pcpt_genes.eq(self.pad_token_id).bool(),
            "gen_key_padding_mask":  padded_gen_genes.eq(self.pad_token_id).bool(),
        }
